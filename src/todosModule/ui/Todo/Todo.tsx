  ...
  function Todo({id, completed, title}: TodoProps) {
    ...
    return (
      ...
      <li className={styles.item}>
        <input className={styles.checkbox }type="checkbox" checked={completed} onChange={handleToggleTodo} />
        <h3 className={ completed ? styles.completed : '' }>{title}</h3>
        <button className={styles.trashButton} onClick={handleDeleteTodo}><TrashLogo /></button>
      </li>
      </>
